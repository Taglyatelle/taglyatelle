...
def test_git_provider_synchronize_changelog(mocker, github_app):
    mock_strategy = mocker.Mock()
    mock_strategy.invoke_llm.return_value = "fake_llm_response"
    mocker.patch.object(github_app._builder, "build", return_value=mock_strategy)
    github_app.set_llm_strategy(provider="openai", model="gpt-4", temperature=0.5)
    result = github_app.synchronize_changelog(content=[{"fake_key": "fake_value"}])
    assert result == "fake_llm_response"


def test_git_provider_bump_version(mocker, github_app):
    mock_strategy = mocker.Mock()
    mock_strategy.invoke_llm.return_value = "0.2.0"
    mocker.patch.object(github_app._builder, "build", return_value=mock_strategy)
    github_app.set_llm_strategy(provider="openai", model="gpt-4", temperature=0.5)
    mocker.patch.object(github_app, "get_current_tag", return_value="0.1.0")
    result = github_app.bump_version(changelog="fake_changelog")
    assert result == "0.2.0"


def test_git_provider_check_licenses(mocker, github_app):
    mock_strategy = mocker.Mock()
    mock_strategy.invoke_llm.return_value = "| Package | License | Severity |\n|---------|---------|----------|\n| numpy | BSD-3 | 🟢 Low |"
    mocker.patch.object(github_app._builder, "build", return_value=mock_strategy)
    github_app.set_llm_strategy(provider="openai", model="gpt-4", temperature=0.5)
    mocker.patch.object(
        github_app,
        "get_file_content",
        side_effect=lambda path, ref: "fake_content"
        if path in github_app.files_to_check
        else None,
    )
    result = github_app.check_licenses(branch="fake_branch")
